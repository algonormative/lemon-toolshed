...
| `HOUSE_PAYERS` | yes | comma-separated wallet addresses whose payments read as a 🧪 test rather than a 🍋💰 sale. Compared lowercased, so a base58 Solana address matches in any case. Holds both house buyers — the EVM one and the Solana one. Non-secret: these are public chain addresses, and it lives in `wrangler.toml`. Also the source of the `house` flag on every analytics event. The convention is written up at [Tag your own traffic](https://lemon-agent.dev/guides/payments/house-traffic-tagging/). |
...
**Discovery carries the address, from 2026-09-10.** `GET /.well-known/x402` is
the document read *before* anything is paid, and its `accepts` entries now name
the `payTo` of each rail — the house's estate-watch collector found this service
the odd one out against kino402, penny402 and parallax, all of which carried it.
The addresses are runtime configuration (`PAYTO` is a secret, `PAYTO_SOLANA` a
var), so the build bakes nothing: the zone Worker substitutes them **per network
at serve time**, out of the same env the 402 envelope is built from, which is why
the two documents cannot disagree and why rotating an address needs no rebuild.
It stays a pure in-memory transform — a read-only document `GET` makes no fetch,
no KV read and no D1 query, and in particular does not make the facilitator's
fee-payer call, so what discovery cannot tell you is whether the Solana rail is
*offerable right now*; the live 402 remains the authority on that. With an
address unconfigured the key is **absent, never empty**: this is JSON a machine
compares, and `""` is a value someone could try to pay to. The suite asserts
discovery's `payTo` against the live envelope's on both rails
(`test/wellknown-payto.test.mjs`, `test/x402.test.mjs`) and that the built bytes
still carry no address at all (`test/surfaces.test.mjs`).
